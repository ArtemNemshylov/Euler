...
# -*- coding: utf-8 -*-
"""
train_vacancy_classifier.py (no-args, full + robust stratify/pos_weight order)
- Вхід:
...
- Лосс: BCEWithLogitsLoss (+ pos_weight)
- Метрики: accuracy, precision, recall, F1, ROC-AUC, LogLoss
- Шедулер: cosine, warmup_ratio=0.1
- EarlyStopping(patience=3)
- Після тренування:
    eval_metrics.json, best_threshold.json, predict_is_vacancy.py
"""
...
def build_dataset(positives_path: str, negatives_path: str, text_field: Optional[str] = None) -> Dataset:
    pos_raw = _read_jsonl(positives_path)  # очікуємо {"url": "...", "text": "..."}
    neg_raw = _read_jsonl(negatives_path)  # структура довільна
...
def compute_class_pos_weight_from_dataset(dataset: Dataset) -> Optional[float]:
    ...
    y_raw = dataset[col]
    # ClassLabel -> ints
    y = [int(dataset.features[col].int2str(v) != "not") if isinstance(dataset.features.get(col), ClassLabel) else int(v) for v in y_raw] \
        if isinstance(dataset.features.get(col), ClassLabel) else [int(v) for v in y_raw]
    pos = sum(y)
    neg = len(y) - pos
    if pos == 0 or neg == 0:
        return None
...
def main():
    ...
    try:
        ...
        train_idx, test_idx = train_test_split(idx, test_size=VAL_SIZE, random_state=SEED, stratify=y)
        dsd = DatasetDict({"train": ds.select(train_idx), "test": ds.select(test_idx)})

    print(f"[INFO] Train size: {len(dsd['train'])} | Val size: {len(dsd['test'])}")

    # ======== ВАЖЛИВО: pos_weight рахуємо ДО мапінгу/перейменувань ========
    pos_weight_value = None
    if USE_CLASS_WEIGHT:
        pos_weight_value = compute_class_pos_weight_from_dataset(dsd["train"])
    ...
    if FREEZE_EMBEDDINGS:
        if hasattr(base_model, "roberta"):
            for p in base_model.roberta.embeddings.parameters():
                p.requires_grad = False
        elif hasattr(base_model, "xlm_roberta"):
            for p in base_model.xlm_roberta.embeddings.parameters():
                p.requires_grad = False
